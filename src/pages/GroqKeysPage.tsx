...
import { ApiError, createGroqKey, listGroqKeys } from '../lib/api'
import { formatDate } from '../lib/format'
import { useAuth } from '../context/AuthContext'
import type { GroqKeyListItem } from '../types'

export function GroqKeysPage() {
  const { apiBase, adminAuth } = useAuth()
  const [includeInactive, setIncludeInactive] = useState(true)
  const [items, setItems] = useState<GroqKeyListItem[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const list = await listGroqKeys(apiBase, adminAuth, { includeInactive })
      setItems(list)
    } catch (e) {
      if (e instanceof ApiError && e.status === 401) {
        ...
        setError('Failed to load Groq keys.')
      }
      setItems([])
    } finally {
      setLoading(false)
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    setError(null)
    try {
      await createGroqKey(apiBase, adminAuth, {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-white">Groq API keys</h1>
          <p className="mt-1 text-sm text-zinc-400">
            Manage stored keys for the desktop app via{' '}
            <span className="font-mono text-zinc-500">GET /api/client-config</span> (user JWT — not
            this admin UI). List shows masked values only.
          </p>
        </div>
      ...
      <div className="mt-6 space-y-3 rounded-2xl border border-zinc-700/80 bg-zinc-900/50 p-4 text-sm text-zinc-300">
        ...
        <p>
          <span className="font-medium text-zinc-200">Inactive:</span> inactive rows stay in the
          database. Client config can still include them for signed-in desktop users — turning off
          Active does not hide the secret from the app; it only flags the row.
        </p>
        <p>
          <span className="font-medium text-amber-200/90">Windows app:</span> at least one Groq key
          row must exist or the client receives <span className="font-mono text-xs">503</span>{' '}
          <span className="font-mono text-xs">MISSING_GROQ_KEYS</span>.
        </p>
      </div>
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-zinc-800">
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-zinc-500">
                    No Groq keys yet. Add one to get started.
                  </td>
                </tr>
              ) : (
                items.map((row) => (
      ...
      >
        <form id="groq-create-form" onSubmit={onCreate} className="space-y-4">
