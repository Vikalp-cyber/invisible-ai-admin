...
import { ApiError, deleteDeepgramKey, getDeepgramKey, patchDeepgramKey } from '../lib/api'
import { formatDate } from '../lib/format'
import { useAuth } from '../context/AuthContext'
import type { DeepgramKeyDetail, DeepgramKeyPatchBody } from '../types'

export function DeepgramKeyDetailPage() {
  const { keyId = '' } = useParams()
  const navigate = useNavigate()
  const { apiBase, adminAuth } = useAuth()

  const [key, setKey] = useState<DeepgramKeyDetail | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const k = await getDeepgramKey(apiBase, adminAuth, keyId)
      setKey(k)
      setLabel(k.label)
      setNotes(k.notes ?? '')
  ...
  async function onSave(e: React.FormEvent) {
    ...
    setError(null)
    try {
      const body: DeepgramKeyPatchBody = {
        label: label.trim(),
      ...
      const updated = await patchDeepgramKey(apiBase, adminAuth, keyId, body)
      setKey(updated)
      setRotateKey('')
      setShowSecret(false)
    } catch (err) {
  ...
  async function onDelete() {
    ...
    setError(null)
    try {
      await deleteDeepgramKey(apiBase, adminAuth, keyId)
      navigate('/deepgram-keys', { replace: true })
    } catch (err) {
      if (err instanceof ApiError) setError(err.message)
